...
def main() -> int:
    ...
    parser.add_argument("--include-published", action="store_true", help="Rescan rows already published by the current scanner build.")
    ...
    published = {} if args.include_published else published_artifacts(args.publication_url, current_build)
    ...
    print(json.dumps({"corpus": len(rows), "already_current": len(rows) - len(pending), "dispatched": 0 if args.dry_run else len(pending), "scanner_build": current_build}, sort_keys=True))
    return 0


def published_artifacts(url: str, current_build: str) -> dict[str, str]:
    with urllib.request.urlopen(url, timeout=60) as response:
        body = json.loads(response.read().decode())
    published: dict[str, str] = {}
    for row in body.get("rows") or []:
        scan = row.get("scan") or {}
        if scan.get("scanner_build") != current_build or scan.get("score_schema_version") != "2" or scan.get("coverage_percent") != 100:
            continue
